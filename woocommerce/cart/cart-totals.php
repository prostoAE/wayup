...
<div class="cart_totals <?php echo (WC()->customer->has_calculated_shipping()) ? 'calculated_shipping' : ''; ?> result">
  ...
  <div class="result__title"><?php esc_html_e('Totals', 'woocommerce'); ?></div>

  <div class="cart-subtotal result__row">
    <div class="result__name"><?php esc_html_e('Subtotal', 'woocommerce'); ?></div>
  ...
  <?php elseif (WC()->cart->needs_shipping() && 'yes' === get_option('woocommerce_enable_shipping_calc')) : ?>
    ...
    <div class="shipping result__row">
      <div class="result__name"><?php esc_html_e('Shipping', 'woocommerce'); ?></div>
  ...
  if (wc_tax_enabled() && !WC()->cart->display_prices_including_tax()) {
    ...
    if ('itemized' === get_option('woocommerce_tax_total_display')) {
      foreach (WC()->cart->get_tax_totals() as $code => $tax) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited
        ?>
        <div class="tax-rate tax-rate-<?php echo esc_attr(sanitize_title($code)); ?> result__row">
          <div class="result__name"><?php echo esc_html($tax->label) . $estimated_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
      ...
      <div class="tax-total result__row">
        <div class="result__name"><?php echo esc_html(WC()->countries->tax_or_vat()) . $estimated_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
