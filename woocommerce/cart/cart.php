        ...
        <div class="woocommerce-cart-form__cart-item <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?> cart__row">
          ...
          <div class="cart__block cart__block_first">
            ...
            if (!$product_permalink) {
              ...
              printf('<a class="cart__image" href="%s">%s</a>', esc_url($product_permalink), $thumbnail); // PHPCS: XSS ok.
            }
            ?>
            ...
            <div class="product-name cart__desc sale">
              ...
              <?php
              if ($_product->is_on_sale() && (float)$_product->get_regular_price() > 0) { ?>
                <span class="discount">
              ...
              if (!$product_permalink) {
                ...
                echo wp_kses_post(apply_filters('woocommerce_cart_item_name', sprintf('<a class="cart__product" href="%s">%s</a>', esc_url($product_permalink), $_product->get_name()), $cart_item, $cart_item_key));
              }
          ...
          <div class="cart__block cart__block_second">
            ...
            echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
              ...
              sprintf(
                '<div class="product-remove cart__action"><a href="%s" aria-label="%s" data-product_id="%s" data-product_sku="%s"><button class="cart__del" type="submit" value="cart/remove">✖</button></a></div>',
    ...
    <div class="promo">
      <div class="coupon">
        <?php if (wc_coupons_enabled()) { ?>
          <input type="text" name="coupon_code" class="input-text promo__input log__input" id="coupon_code" value=""
                 placeholder="<?php esc_attr_e('Coupon code', 'woocommerce'); ?>"/>
          <button type="submit" class="btn promo__btn" name="apply_coupon"
                  value="<?php esc_attr_e('Apply coupon', 'woocommerce'); ?>"><?php esc_attr_e('Apply coupon', 'woocommerce'); ?></button>
        <?php } ?>
        <button type="submit" class="btn promo__btn" name="update_cart" style="margin-left: 10px"
